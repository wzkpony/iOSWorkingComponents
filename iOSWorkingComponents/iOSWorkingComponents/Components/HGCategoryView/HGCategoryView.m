...
///游标样式类型
@property (assign ,nonatomic) VernierType vernierType;
...
- (void)layoutSubviews {
    ...
    [self.titleContentView configCornerRadius:self.titleContentView.jk_height/2.0];
}

- (void)setSelected:(BOOL)selected {
    [super setSelected:selected];
    self.titleLabel.textColor = selected ? self.titleSelectedColor : self.titleNormalColor;
    if (_vernierType == FillContentView) {
        if (selected) {
            [self.titleContentView crosswiseStartColor:App_UICOLOR_HEX(@"#FE635B") endColor:App_UICOLOR_HEX(@"#FC3455")];
        }
        else {
            [self.titleContentView crosswiseStartColor:[UIColor clearColor] endColor:[UIColor clearColor]];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.vernierView.backgroundColor = self.titleSelectedColor;
        [self setupSubViews];
    }
    return self;
}
...
#pragma mark - Public Method 滚动scroll调用
- (void)scrollToTargetIndex:(NSUInteger)targetIndex sourceIndex:(NSUInteger)sourceIndex percent:(CGFloat)percent {
    ...
    if (targetCell) {
        ...
        [self.vernierWidthConstraint uninstall];
        [self.vernierView mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)setupSubViews {
    ...
    [self.collectionView addSubview:self.vernierView];
    ...
    [self.vernierView mas_makeConstraints:^(MASConstraintMaker *make) {
        CGFloat collectionViewHeight = self.height - ONE_PIXEL * 2;
        make.top.mas_equalTo(collectionViewHeight - self.vernierHeight - _vernierBottom);
    ...
    [self.vernierView addSubview:self.imageFitleBottomIcon];
    [self.imageFitleBottomIcon mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.vernierView.mas_centerX);
...
- (void)updateVernierLocation {
    ...
    if (cell) {
        CGRect targetVernierFrame = [self vernierFrameWithIndex:self.selectedIndex];

        [self.vernierLeftConstraint uninstall];
        [self.vernierWidthConstraint uninstall];
        [self.vernierView mas_updateConstraints:^(MASConstraintMaker *make) {
            if (self.fixedVernierWidth) {
                self.vernierLeftConstraint = make.left.mas_equalTo(targetVernierFrame.origin.x - self.fixedVernierWidth/2.0);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    CGFloat width = [self getWidthWithContent:self.titles[indexPath.item]] + 30;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.vernierType = self.vernierType;
    return cell;
}
...
- (void)setHeight:(CGFloat)height {
    ...
    [self.vernierView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.height - self.vernierHeight - ONE_PIXEL - _vernierBottom);
    }];
}
...
- (void)setVernierHeight:(CGFloat)vernierHeight {
    ...
    [self.vernierView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.height - self.vernierHeight - ONE_PIXEL - _vernierBottom);
        make.height.mas_equalTo(vernierHeight);
    }];
}
...
- (void)setVernierType:(VernierType)vernierType {
    _vernierType = vernierType;
    if (self.vernierType == LineView) {
        self.imageFitleBottomIcon.image = nil;
        self.vernierView.backgroundColor = self.titleSelectedColor;
        _vernierBottom = (_vernierBottom <= 0)?10:_vernierBottom;
    }
    else if (self.vernierType == FillContentView) {
        self.vernierHeight = 0;
    }
    [self.collectionView reloadData];
}
...
- (UIImageView *)imageFitleBottomIcon {
    if (_imageFitleBottomIcon == nil) {
        _imageFitleBottomIcon = [[UIImageView alloc] init];
        _imageFitleBottomIcon.image = [UIImage imageNamed:@"fitleBottomIcon"];
    }
    return _imageFitleBottomIcon;
}
