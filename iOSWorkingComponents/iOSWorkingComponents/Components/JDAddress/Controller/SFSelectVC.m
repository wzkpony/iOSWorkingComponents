...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_lastSelctcModel == clickModel){
        
    }else if (_lastSelctcModel.depth == clickModel.depth){
        if (_lastSelctcModel) {
            NSInteger row = [self.midelModels indexOfObject:_lastSelctcModel];
...
- (void)onClickBtn:(NSInteger)index{
    if (index < 0) {
        if (self.delegate) {
            NSString* firstRegionId = @"";
            NSString*  lastRegionId = @"";
            NSString *adressStr = @"";
            ...
            if (_lastSelctcModel != NULL) {
                  [_selectArr addObject:_lastSelctcModel];
            }
