...
/**
 ...
 @param plainData plainData解密后的data
 @param signature 服务的签名
 @return 是否验签成功
...
+ (BOOL)rsaSHA1VertifyingData:(NSString *)plainData withSignature:(NSString *)signature{
    ...
    NSData *signatureBytes = [signature base64DecodedData];
    
    SecKeyRef publicKey = [self addPublicKey:PublicKey];
    if((!publicKey)||(!signatureBytes)||(!plainTextBytes)){
        return NO;
    }
    
    size_t signedHashBytesSize = SecKeyGetBlockSize(publicKey);
    const void* signedHashBytes = [signatureBytes bytes];
    
    size_t hashBytesSize = CC_SHA1_DIGEST_LENGTH;
    uint8_t* hashBytes = malloc(hashBytesSize);
    if (!CC_SHA1([plainTextBytes bytes], (CC_LONG)[plainTextBytes length], hashBytes)) {
        return NO;
    }
