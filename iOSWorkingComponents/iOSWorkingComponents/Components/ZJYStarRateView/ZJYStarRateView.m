...
@property (nonatomic,assign)CGFloat startWidth; //星大小，方形的
...
-(instancetype)initWithFrame:(CGRect)frame withWidth:(CGFloat)width{
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = 5;
        _startWidth = width?width:20;
        _starSize = CGSizeMake(_startWidth, _startWidth);
        _starSpaceing = 10;
...
-(void)createStarView{
    ...
    if (_currentScore > 0) {
        foregroundStarViewWidth = _currentScore * _startWidth + (ceilf(_currentScore) - 1) * _starSpaceing;
    } else {
...
- (UIView *)createStarViewWithImage:(NSString *)imageName {
    ...
    for (NSInteger i = 0; i < self.numberOfStars; i ++)
    {
        ...
        imageView.frame = CGRectMake(i * (_startWidth + _starSpaceing), 0, _startWidth, self.bounds.size.height);
...
- (void)userTapRateView:(UITapGestureRecognizer *)gesture {
    ...
    //一个星+一个间距为单位 向下取整
    CGFloat floorStarScore = floorf(offset / (_startWidth + _starSpaceing));
    
    //多于一个完整星（星宽+间距）外的宽度
    CGFloat excessWidth = offset - floorStarScore * (_startWidth + _starSpaceing);
    
    //真实星数目
    CGFloat realStarScore = (offset - floorStarScore * _starSpaceing) / _startWidth;
    ;
    
    if (excessWidth > _startWidth) { //多的是间距。向下取整
...
- (void)layoutSubviews {
    ...
    [UIView animateWithDuration:animationTimeInterval animations:^{
        ...
        if (self->_currentScore > 0) {
            foregroundStarViewWidth = self->_currentScore * self->_startWidth + (ceilf(self->_currentScore) - 1) * self->_starSpaceing;
        } else {
            foregroundStarViewWidth = 0;
        }
